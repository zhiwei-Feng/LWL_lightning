...
from ltr.data.image_loader import opencv_loader, imread_indexed
...
class VOSMeta:
    ...
    def _transpose_nested_dict(d):
        ...
        # focus: 这样得到的d2 中key就是sequence所有帧的目标， 巧妙
        return d2

    def select_split(self, dataset_name, split):
        ltr_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), '..')
        sequences = set([s.strip() for s in
                         open(os.path.join(ltr_path, 'data_specs', dataset_name + '_' + split + '.txt')).readlines()])
...
class VOSDatasetBase(BaseVideoDataset):
    ...
    def get_paths_and_bboxes(self, sequence_info):
        ...
        for obj_id in sequence_info['object_ids']:
            gt_bboxes[obj_id] = np.array(
                [all_bboxes.get(frame, {}).get(obj_id, [-1, -1, -1, -1]) for frame in frame_names])
